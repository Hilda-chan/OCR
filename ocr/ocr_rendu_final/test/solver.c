...
int Is_valid_row(unsigned long sudoku[], size_t x){
    unsigned long value = sudoku[x];
    size_t start = x - x%9;
...
int Is_valid_col(unsigned long sudoku[], size_t x){
    unsigned long value = sudoku[x];
    size_t start = x % 9;
    for(size_t j = start; j < 81; j++){
        if(sudoku[j] == value){
            if (j != x)
                return -1;
        }
...
int Is_valid_block(unsigned long sudoku[], size_t x){
    unsigned long value = sudoku[x];
    size_t start = x - x%27 + x % 9 /3 *3;
    for(size_t k = start; k < start + 27; k++){
...
int Is_valid(unsigned long sudoku[]){
    for(size_t i = 0; i < 81; i++){
        if(sudoku[i] > 10)
            return -1;
        if (Is_valid_row(sudoku, i) == -1)
            return -1;
        if (Is_valid_col(sudoku, i) == -1)
            return -1;
        if (Is_valid_block(sudoku, i) == -1)
            return -1;
    }
    return 0;
}
...
void __del_proba(unsigned long* p[], size_t poz, unsigned long value) {
    ...
    if (p[0][poz] < 10) {       // sudoku[poz] < 10
        p[1][*p[2]] = poz; // nkv[nkv_poz]] = poz
        *p[2] += 1;     // nkv_poz += 1
    }
}
...
size_t first_unknown_value(unsigned long sudoku[]){
    size_t i = 0;
    while (sudoku[i] < 10){
        i++;
    }
...
unsigned long* copy_sudoku(unsigned long sudoku[]){
    unsigned long copy[81];
    for(size_t i = 0; i < 81; i++){
        copy[i] = sudoku[i];
    }
}
...
void resolve(unsigned long* p[], size_t i){ 
    while( i < 81){
        for(size_t i = 0; i<81; i++){
            printf("%lu,", p[0][i]);
        }
        ...
        if (p[1][i] == 81){
            size_t index = first_unknown_value(p[0]);
            unsigned long value = p[0][index];
            unsigned long* save_sudoku =  p[0];
            p[1][i] = index;
            while(value > 0 && Is_valid(p[0]) == -1){
                    p[0] = save_sudoku;
                    p[0][index] = value%10;
                    value = value/10;
                    resolve(p, i);
